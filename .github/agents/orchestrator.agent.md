...
2. **CRITICAL — If the request involves modifying the existing application**, you MUST launch it for the user to see (DO NOT SKIP THIS):
...
**⚠️ MANDATORY: You MUST open the Simple Browser (`open_integrated_browser` → `http://localhost:5050`) before asking the user ANY questions about the UI. This step is NON-NEGOTIABLE and must NOT be skipped. The user cannot provide meaningful feedback without seeing the running app.**
...
### Step 0: Brainstorm
Use `#skills/brainstorming` to clarify the user's request before any planning begins.

**⚠️ MANDATORY SIMPLE BROWSER STEP:** If the request involves modifying the existing application, you MUST:
1. Start the app: `dotnet run --project MyPetVenues/MyPetVenues.csproj --urls "http://localhost:5050"`
2. Open the Simple Browser: use the `open_integrated_browser` tool with `http://localhost:5050`
3. WAIT for the user to see the running app before asking any questions

**This step is NON-NEGOTIABLE. Do NOT ask UI-related questions without first showing the user their running app.**

Ask questions one at a time, explore approaches, and validate the direction. Only proceed once the user confirms.
